...
Although both files describe the same engineering assembly, they have completely different purposes. Every CAD component receives an initial **Component Frame** automatically generated by GSPL-01. The frame is located at the geometric center of the component Bounding Box and is stored in `1_model_database.json`. During GSPL-02 the engineer may redefine this frame if required by the simulation model.
```
...
The spreadsheet is automatically populated by GSPL-01 with default engineering values, drop-down lists and computed geometric information. The engineer only modifies the fields required to define the simulation model.
