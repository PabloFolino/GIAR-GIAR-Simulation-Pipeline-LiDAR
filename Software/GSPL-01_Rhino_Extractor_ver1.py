...
class RhinoExtractor:
    ...
    def generate_assembly_template(self):
        ...
        for source in self.database["components"]:
            ...
            default_shape = {
                # Is the index of the object, A componet may be more one object
                "id":1,                 
        ...
        for component in assembly["components"]:
            ...
            if component["models"]["simulation"]:
                statistics["simulation_models"] += 11
    ...
    def create_objects_sheet(self, workbook):
        ...
        for component in self.database["components"]:
            ...
            ws.cell(row=row, column=6).value = reference_frame

            # ----------------------------------------------
            # Object Properties
            # ----------------------------------------------

            ws.cell(row=row, column=7).value = dynamic
            ws.cell(row=row, column=8).value = respondable
            ws.cell(row=row, column=9).value = visible
            ws.cell(row=row, column=10).value = collidable
            ws.cell(row=row, column=11).value = detectable
            ws.cell(row=row, column=12).value = measurable

            # ----------------------------------------------
            # Position
            # ----------------------------------------------

            ws.cell(row=row, column=13).value = position[0]
            ws.cell(row=row, column=14).value = position[1]
            ws.cell(row=row, column=15).value = position[2]

            # ----------------------------------------------
            # Orientation
            # ----------------------------------------------

            ws.cell(row=row, column=16).value = orientation[0]
            ws.cell(row=row, column=17).value = orientation[1]
            ws.cell(row=row, column=18).value = orientation[2]

            # ----------------------------------------------
            # Bounding Box
            # ----------------------------------------------

            ws.cell(row=row, column=19).value = bbox_min[0]
            ws.cell(row=row, column=20).value = bbox_min[1]
            ws.cell(row=row, column=21).value = bbox_min[2]

            ws.cell(row=row, column=22).value = bbox_size[0]
            ws.cell(row=row, column=23).value = bbox_size[1]
            ws.cell(row=row, column=24).value = bbox_size[2]
        ...
        ws.auto_filter.ref = f"A1:AD{row-1}"
        # --------------------------------------------------
        # Column Widths
        # --------------------------------------------------
    ...
    def apply_excel_validations(self, workbook):
        ...
        dv_object_boolean = DataValidation(
            type="list",
            formula1="=Boolean",
            allow_blank=False
        )
        ...
        ws_objects.add_data_validation(dv_object_boolean)
        ...
        for row in range(2, ws_components.max_row + 1):
            ...
            # Component Type
            dv_component_type.add(
                ws_components[f"C{row}"]
            )

            # Parent Name
            dv_component_parent.add(
                ws_components[f"F{row}"]
            )

            # Visual
            dv_component_boolean.add(
                ws_components[f"H{row}"]
            )

            # Simulation
            dv_component_boolean.add(
                ws_components[f"I{row}"]
            )

            # Status
            dv_component_status.add(
                ws_components[f"J{row}"]
            )
        ...
        for row in range(2, ws_objects.max_row + 1):
            ...
            # Enabled
            dv_object_boolean.add(
                ws_objects[f"E{row}"]
            )

            # Reference Frame
            dv_reference_frame.add(
                ws_objects[f"F{row}"]
            )
